...
pub async fn notify(http: &Client, id: &Identity, relay: &str, deliveries: Vec<Delivery>) -> Result<NotifyResp> {
...
pub async fn fetch_pairing(http: &Client, id: &Identity, relay: &str, token: &str) -> Result<Option<Vec<u8>>> {
    ...
    match resp.status() {
        ...
        s => bail!("pairing fetch failed: {} {}", s, resp.text().await.unwrap_or_default()),
    }
}

pub async fn delete_device(http: &Client, id: &Identity, relay: &str, device_id: &str) -> Result<()> {
