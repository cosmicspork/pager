...
async fn full_bridge_relay_device_flow() {
    ...
    let got_blob: pager_proto::SealedBlob =
        serde_json::from_slice(&fetched.bytes().await.unwrap()).unwrap();
    let opened: Enrollment = serde_json::from_slice(
        &pager_proto::open_blob(&bridge, &got_blob, token.as_bytes()).unwrap(),
    )
    .unwrap();
    ...
    let sealed =
        pager_proto::seal_to(&device_id, &serde_json::to_vec(&notif).unwrap(), NOTIFY_AAD).unwrap();
    let delivery = Delivery {
        id: device_id.clone(),
        payload: B64.encode(serde_json::to_vec(&sealed).unwrap()),
    };
    ...
    assert_eq!(
        body.failed, 1,
        "but the relay routed to the registered subscription"
    );
    assert!(
        body.delivered.is_empty(),
        "nothing was accepted by the push service"
    );
    ...
    assert!(
        statuses[0].can_ack,
        "the device registered a signing key at enrollment"
    );
    assert!(statuses[0].last_ack.is_some(), "the ack was recorded");
    assert!(
        statuses[0].last_shown.is_some(),
        "and it reported the alert displayed"
    );
    assert!(
        statuses[0].last_push.is_none(),
        "the fake endpoint never accepted a push"
    );
